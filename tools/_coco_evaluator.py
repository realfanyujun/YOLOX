    ...
    def evaluate(
        ...
        for cur_iter, (imgs, _, info_imgs, ids) in enumerate(
            ...
            with torch.no_grad():
                ...
                img_hs = info_imgs[0].tolist()
                img_ws = info_imgs[1].tolist()
                info_imgs_tuple = [k for k in zip(img_hs, img_ws)]
                ...
                mot_root = "/mnt/data2/UAVDT/vid/val"
                vids = os.listdir(mot_root)
                for i, output_results in enumerate(outputs_):
                    txt_name = "img" + 
                    for vid in vids:
                        if "img"
                    file_name = self.dataloader.dataset.coco.imgs[ids[i].item()]["file_name"].split('.')[0]
                    vid_name = file_name.split('_')[0]
                    img_id = file_name.split('_')[1]
                    #img_path = os.path.join(self.dataloader.dataset.data_dir, self.dataloader.dataset.name, file_name)
                    img_path = os.path.join("/mnt/data2/UAVDT/vid/val", vid_name, "img", img_id)
                
                #if output_results is None:
						
                    img_h, img_w = info_imgs_tuple[i]#.tolist()
                    ...
                    with open(img_path[:-4] + '.txt', 'w') as f:	
                        for tlwh, s, c in zip(bboxes, scores, classes):
                            x1, y1, w, h = tlwh
                            line = save_format.format(x1=x1, y1=y1, w=w, h=h, s=s)
                            f.writelines(line)
        ...
        with open(f'det_db_uavdt_val.json', 'w') as f1:
            json.dump(det_db, f1)
